...
export class HebergementComponent implements OnInit, AfterViewInit {
	@Input() criteres: any;
  ...
  ngOnInit() {
    ...
    this.criteres = this.hebergementService.selectCriteres(this.criteres, CategoryLabelEnum.HEBERGEMENT);
    console.log('selected criteres', this.criteres);
    this.criteres = this.hebergementService.removePrefix(this.criteres, CategoryLabelEnum.HEBERGEMENT);
    this.parsedCriteres = this.hebergementService.initFormValuesWithCriteria(this.criteres);
    this.buildForm();
  }
  ngAfterViewInit() {
  ...
  async onAddOption(field: DynamicFormFieldInterface) {
    this.parsedCriteres = await this.hebergementService.addOption(this.parsedCriteres, field);
    this.buildForm();
  ...
  formSubmitted() {
    ...
    if (this.checkBoxChangesAreChecked()) {
      if (formIsValid) {
        ...
        const dataObject = this.hebergementService.mapFormValueToCriteria(formValue, this.criteres);
