...
import { ChoregraphePricingService } from './choregraphe-pricing.service';
...
@Component({
  selector: 'app-choregraphe-pricing',
  templateUrl: './choregraphe-pricing.component.html',
  styleUrls: ['./choregraphe-pricing.component.scss']
})
export class ChoregraphePricingComponent implements OnInit, AfterViewInit {
	@Input() criteres: any;
  ...
  currentEmail = '';
	constructor(private readonly choregrapheService: ChoregraphePricingService,
  ...
  ngOnInit() {
    ...
    this.criteres = this.choregrapheService.selectCriteres(this.criteres, CategoryLabelEnum.CHOREGRAPHE);
    console.log('selected criteres', this.criteres);
    this.criteres = this.choregrapheService.removePrefix(this.criteres, CategoryLabelEnum.CHOREGRAPHE);
    this.parsedCriteres = this.choregrapheService.initFormValuesWithCriteria(this.criteres);
    this.buildForm();
  }
  ngAfterViewInit() {
  ...
  async onAddOption(field: DynamicFormFieldInterface) {
    this.parsedCriteres = await this.choregrapheService.addOption(this.parsedCriteres, field);
    this.buildForm();
  ...
  formSubmitted() {
    ...
    if (this.checkBoxChangesAreChecked()) {
      if (formIsValid) {
        ...
        const dataObject = this.choregrapheService.mapFormValueToCriteria(formValue, this.criteres);
        this.companyService.putServiceByEmailAndService(CategoryLabelEnum.CHOREGRAPHE,
          this.currentEmail,
