...
import { HebergementPricingService } from './hebergement-pricing.service';
import { AuthService } from '../../../../../../core/auth.service';
import { FieldTypeEnum } from '../../../../../shared/components/dynamic-form-builder/models/field-type.enum';
import { DialogService } from '../../../../services/dialog.service';
@Component({
  selector: 'app-hebergement-pricing',
  templateUrl: './hebergement-pricing.component.html',
  styleUrls: ['./hebergement-pricing.component.scss']
})
export class HebergementPricingComponent implements OnInit, AfterViewInit {
	@Input() criteres: any;
  ...
  currentEmail = '';
	constructor(private readonly hebergementPricingService: HebergementPricingService,
  ...
  ngOnInit() {
    ...
    this.criteres = this.hebergementPricingService.selectCriteres(this.criteres, CategoryLabelEnum.HEBERGEMENT);
    console.log('selected criteres', this.criteres);
    this.criteres = this.hebergementPricingService.removePrefix(this.criteres, CategoryLabelEnum.HEBERGEMENT);
    this.parsedCriteres = this.hebergementPricingService.initFormValuesWithCriteria(this.criteres);
    this.buildForm();
  }
  ngAfterViewInit() {
  ...
  async onAddOption(field: DynamicFormFieldInterface) {
    this.parsedCriteres = await this.hebergementPricingService.addOption(this.parsedCriteres, field);
    this.buildForm();
  ...
  formSubmitted() {
    ...
    if (this.checkBoxChangesAreChecked()) {
      if (formIsValid) {
        ...
        const dataObject = this.hebergementPricingService.mapFormValueToCriteria(formValue, this.criteres);
        this.companyService.putServiceByEmailAndService(CategoryLabelEnum.HEBERGEMENT,
          this.currentEmail,
