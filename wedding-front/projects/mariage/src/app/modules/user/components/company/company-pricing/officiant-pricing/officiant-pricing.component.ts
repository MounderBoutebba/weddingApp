...
import { OfficiantPricingService } from './officiant-pricing.service';
...
@Component({
  selector: 'app-officiant-pricing',
  templateUrl: './officiant-pricing.component.html',
  styleUrls: ['./officiant-pricing.component.scss']
})
export class OfficiantPricingComponent implements OnInit, AfterViewInit {
	@Input() criteres: any;
  ...
  currentEmail = '';
	constructor(private readonly officiantService: OfficiantPricingService,
...
ngOnInit() {
  ...
  this.criteres = this.officiantService.selectCriteres(this.criteres, CategoryLabelEnum.OFFICIANT);
  console.log('selected criteres', this.criteres);
  this.criteres = this.officiantService.removePrefix(this.criteres, CategoryLabelEnum.OFFICIANT);
  this.parsedCriteres = this.officiantService.initFormValuesWithCriteria(this.criteres);
  this.buildForm();
}
ngAfterViewInit() {
...
async onAddOption(field: DynamicFormFieldInterface) {
  this.parsedCriteres = await this.officiantService.addOption(this.parsedCriteres, field);
  this.buildForm();
...
formSubmitted() {
  ...
  if (this.checkBoxChangesAreChecked()) {
    if (formIsValid) {
      ...
      const dataObject = this.officiantService.mapFormValueToCriteria(formValue, this.criteres);
      this.companyService.putServiceByEmailAndService(CategoryLabelEnum.OFFICIANT,
        this.currentEmail,
