import { Component, OnInit, Optional, Inject, ViewChild, ElementRef, PLATFORM_ID, AfterViewInit } from '@angular/core';
import { Observable } from 'rxjs';
import { FormGroup, FormBuilder, Validators, FormControl, ValidationErrors } from '@angular/forms';
import { MatDialogRef, MAT_DIALOG_DATA } from '@angular/material/dialog';
import { BreakpointObserver, Breakpoints } from '@angular/cdk/layout';
import { map } from 'rxjs/operators';
import { BankAccountTypeEnum } from 'projects/mariage/src/app/modules/shared/components/dynamic-form-builder/models/bank-account-type.enum';
...
export class PaymentInfoDialogComponent implements OnInit, AfterViewInit {
	public isHandset$: Observable<boolean>;
	public form: FormGroup;
	public firstDoc: any;
	public secDoc: any;
	public description = 'Mettre à jour mes informations bancaire';
	@ViewChild('autocompleteaddr') autocompleteAddr: ElementRef<HTMLElement>;
	accountTypes = [BankAccountTypeEnum.PARTICULIER, BankAccountTypeEnum.ENTREPRISE];
	countries = [
		'Austria',
		'Belgium',
		'Bulgaria',
	...
	public getPlaceAutocomplete() {
		...
		google.maps.event.addListener(autocomplete, 'place_changed', () => {
			...
			this.form
				.get('location')
				.get('postalCode')
				.setValue(this.getPostalCodeFromAddress(place.address_components));
			this.form
				.get('location')
				.get('address')
	...
	private getPostalCodeFromAddress(addressComponents: any) {
		let postalCode;
	...
	private getCityFromAddress(addressComponents: any) {
		let city;
		addressComponents.forEach(component => {
	...
	public onFileChange(event, input: string) {
		if (event.target.files && event.target.files.length) {
			const file = event.target.files;
			Array.from(file).map((res: File) => {
	...
	buildForm() {
		this.ibanReactive = new FormControl('', [Validators.required, ValidatorService.validateIban]);
		this.form = this.fb.group({
			firstname: [
				this.data.user.firstname,
				Validators.compose([Validators.required, Validators.minLength(3), Validators.maxLength(255)])
			],
			lastname: [
				this.data.user.lastname,
				Validators.compose([Validators.required, Validators.minLength(3), Validators.maxLength(255)])
			],
			dateOfBirth: [new Date(), Validators.compose([Validators.required])],
			location: this.fb.group({
				...
				postalCode: [
					0,
					Validators.compose([Validators.required, Validators.minLength(1), Validators.maxLength(127)])
				],
				city: [
					'',
					Validators.compose([Validators.required, Validators.minLength(2), Validators.maxLength(127)])
			...
			accountType: [BankAccountTypeEnum.PARTICULIER, Validators.compose([Validators.required])],
			bankName: [
				'',
				Validators.compose([Validators.required, Validators.minLength(3), Validators.maxLength(255)])
			],
	...
	save() {
		if (this.form.valid && !!this.firstDoc && !!this.secDoc) {
			const data = 'dummy data';
			const payload = {
				country: this.countryListAllIsoData.find(country => country.name === this.form.get('country').value)
					.code,
				account_holder_name: this.form.get('raison').value,
				account_holder_type: this.form.get('accountType').value,
				account_number: this.form.get('ibanReactive').value,
				address: this.form.get('location').get('address').value,
			...
			this.companyService.updateBankInfo(this.data.user.email, payload).subscribe(res => {
				this.data = { save: true, data };
				this.dialogRef.close(this.data);
			});
		}
	}
	ngOnInit(): void {
		this.buildForm();
		const latlng = { lat: parseFloat(this.data.location.lat), lng: parseFloat(this.data.location.lng) };
		this.getAddress(latlng).then(data => {
			this.form
				.get('location')
				.get('address')
				.setValue(this.data.location.address);
			this.form
				.get('location')
				.get('lat')
				.setValue(this.data.location.lat);
			this.form
				.get('location')
				.get('lng')
				.setValue(this.data.location.lng);
			this.form
				.get('location')
				.get('postalCode')
				.setValue(getPostalCodeFromAddress(data[0].address_components));
			this.form
				.get('location')
				.get('city')
				.setValue(this.getCityFromAddress(data[0].address_components));
		});
	}
...
export function getPostalCodeFromAddress(addressComponents: any) {
	let postalCode;
	console.log(addressComponents);
	addressComponents.forEach(component => {
		if (component.types[0] === 'postal_code') {
